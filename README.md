...
- Содержит функции для извлечения данных об акциях из интернета, расчёта скользящего среднего и средней цены закрытия
  акций за заданный период, вывода уведомления о сильных колебаниях цены акции, а также записи данных в CSV файл.
...
- Запрашивает у пользователя тикер акции и временной период, загружает данные, обрабатывает их и выводит результаты в
  виде графика, а также записывает данные в CSV файл.
...
4. Программа обработает введённые данные, загрузит соответствующие данные об акциях, рассчитает скользящее среднее и
   отобразит график, а также запишет дынные в CSV файл
